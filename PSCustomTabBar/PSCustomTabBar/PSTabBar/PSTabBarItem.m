...
- (void) itemDidLayoutBadgeLabel {
    switch (self.itemModel.itemBadgeStyle) {
        case PSTabBarItemBadgeStyleTopRight:{   // 右上方 默认
...
- (void)itemDidLayoutControl{
    ...
    switch (self.itemModel.itemLayoutStyle) {
        case PSTabBarItemLayoutStyleTopPictureBottomTitle:{         // 上图片下文字 使用图3 文1比
        ...
        case PSTabBarItemLayoutStyleBottomPictureTopTitle:{         // 下图片上文字
        ...
        case PSTabBarItemLayoutStyleLeftPictureRightTitle:{         // 左图片右文字
        ...
        case PSTabBarItemLayoutStyleRightPictureLeftTitle:{         // 右图片左文字
            // 左右布局要重新计算图片宽度和文本高度 比例要按照相反的1：3计算
            iconImgFrame.size.width = self.frame.size.width * titleLabel_Proportion ;
            titleFrame.size.height = self.frame.size.height - self.itemModel.componentMargin.top - self.itemModel.componentMargin.bottom;
            titleFrame.size.width = self.frame.size.width - // 图片的右下终点坐标 + 边距 + 组件右边距极限
        ...
        case PSTabBarItemLayoutStylePicture:{                       // 单图片占满全部
        ...
        case PSTabBarItemLayoutStyleTitle:{                         // 单标题占满全部
...
//    WeakSelf;//添加动画
    ...
    switch (self.itemModel.interactionEffectStyle) {
        case PSTabBarInteractionEffectStyleNone:{ // 无
        }break;
        case PSTabBarInteractionEffectStyleSpring:{ // 放大放小
        ...
        case PSTabBarInteractionEffectStyleShake:{ // 摇动
            animation.keyPath = @"transform.rotation";
            CGFloat angle = M_PI_4 / 10;
            animation.values = @[@(-angle), @(angle), @(-angle)];
            animation.duration = 0.2f;
        }break;
        case PSTabBarInteractionEffectStyleAlpha:{ // 透明
            animation.keyPath = @"opacity";
            animation.values = @[@1.0,@0.7,@0.5,@0.7,@1.0];
            animation.duration = 0.6;
        }break;
        case PSTabBarInteractionEffectStyleCustom:{ // 自定义动画
...
- (void)setItemModel:(PSTabBarConfiguration *)itemModel{
    ...
    self.badgeLabel.hiddenZero = _itemModel.automaticHidden;
...
- (PSTabBadgeLabel *)badgeLabel {
    if (!_badgeLabel) {
        _badgeLabel = [PSTabBadgeLabel new];
...
@implementation PSTabBarConfiguration
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.selectColor = [UIColor colorWithRed:59/255.0 green:185/255.0 blue:222/255.0 alpha:1];
